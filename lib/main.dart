...
final ValueNotifier<String> glyphAnimationStyleNotifier = ValueNotifier('Breath & Progress');
...
final ValueNotifier<List<String>> glyphCustomChannelsNotifier = ValueNotifier(['A1', 'B1', 'C-All', 'D-All', 'E1']);
final ValueNotifier<int> glyphBreathProgressDurationNotifier = ValueNotifier(65000);
final ValueNotifier<int> glyphBreathProgressIntervalNotifier = ValueNotifier(100);

/// In-Call settings controlled from HomeScreen.
final ValueNotifier<String> inCallAnimationStyleNotifier = ValueNotifier('Breath & Progress');
...
final ValueNotifier<List<String>> inCallCustomChannelsNotifier = ValueNotifier(['A1', 'B1', 'C-All', 'D-All', 'E1']);
final ValueNotifier<int> inCallBreathProgressDurationNotifier = ValueNotifier(65000);
final ValueNotifier<int> inCallBreathProgressIntervalNotifier = ValueNotifier(100);
...
int get _glyphBreathProgressDuration => glyphBreathProgressDurationNotifier.value;
int get _glyphBreathProgressInterval => glyphBreathProgressIntervalNotifier.value;
...
int get _inCallBreathProgressDuration => inCallBreathProgressDurationNotifier.value;
int get _inCallBreathProgressInterval => inCallBreathProgressIntervalNotifier.value;
...
class _NothingDialerAppState extends State<NothingDialerApp> {
  ...
  Future<void> _initGlyph() async {
    ...
    glyphAnimationStyleNotifier.value = prefs.getString('glyph_animation_style') ?? 'Breath & Progress';
    glyphC1C4IntervalNotifier.value = prefs.getInt('glyph_c1c4_interval') ?? 1000;
    glyphCustomIntervalNotifier.value = prefs.getInt('glyph_custom_interval') ?? 1500;
    glyphCustomChannelsNotifier.value = prefs.getStringList('glyph_custom_channels') ?? ['A1', 'B1', 'C-All', 'D-All', 'E1'];
    glyphBreathProgressDurationNotifier.value = prefs.getInt('glyph_breath_progress_duration') ?? 65000;
    glyphBreathProgressIntervalNotifier.value = prefs.getInt('glyph_breath_progress_interval') ?? 100;

    inCallAnimationStyleNotifier.value = prefs.getString('in_call_animation_style') ?? 'Breath & Progress';
    inCallC1C4IntervalNotifier.value = prefs.getInt('in_call_c1c4_interval') ?? 1000;
    inCallCustomIntervalNotifier.value = prefs.getInt('in_call_custom_interval') ?? 1500;
    inCallCustomChannelsNotifier.value = prefs.getStringList('in_call_custom_channels') ?? ['A1', 'B1', 'C-All', 'D-All', 'E1'];
    inCallBreathProgressDurationNotifier.value = prefs.getInt('in_call_breath_progress_duration') ?? 65000;
    inCallBreathProgressIntervalNotifier.value = prefs.getInt('in_call_breath_progress_interval') ?? 100;

    // Initial default dialer check
    try {
      isDefaultDialerNotifier.value = await _controlMethodChannel.invokeMethod<bool>('isDefaultDialer') ?? true;
    } catch (e) {
...
Future<void> _triggerGlyphLights({bool isOutgoing = false, bool isActiveCall = false}) async {
  ...
  _runAnimationLoop(currentSeq, isOutgoing: isOutgoing, isActiveCall: isActiveCall);
...
Future<void> _runAnimationLoop(int seqId, {bool isOutgoing = false, bool isActiveCall = false}) async {
...
Future<void> _runBreathAndProgressLoop(int seqId, {bool isActiveCall = false}) async {
  int elapsedMillis = 0;
  final durationMillis = isActiveCall ? _inCallBreathProgressDuration : _glyphBreathProgressDuration;
  final interval = isActiveCall ? _inCallBreathProgressInterval : _glyphBreathProgressInterval;
  ...
  while (_glyphAnimRunning && _animSequenceId == seqId) {
    try {
      ...
      if (_isPhone1) {
        builderOn.buildChannelA().buildChannelB().buildChannelC().buildChannelE();
...
Future<void> _runCustomBreathLoop(int seqId, {bool isActiveCall = false}) async {
  while (_glyphAnimRunning && _animSequenceId == seqId) {
    try {
      final interval = isActiveCall ? _inCallCustomInterval : _glyphCustomInterval;
